...
export async function getTransactions(filters?: TransactionFilters): Promise<TransactionsResponse> {
  try {
    ...
    const url = `${API.baseUrl}${API.endpoints.transactions}${queryString}`
...
export async function getTransactionById(transactionId: string): Promise<Transaction> {
  try {
    const url = `${API.baseUrl}${API.endpoints.transactions}/${transactionId}`
 ...
 */
export async function getWalletBalance(currency?: string): Promise<any> {
  try {
    const queryParams = new URLSearchParams()
    if (currency) queryParams.append("currency", currency)

    const queryString = queryParams.toString() ? `?${queryParams.toString()}` : ""
    const url = `${API.baseUrl}${API.endpoints.balance}${queryString}`
