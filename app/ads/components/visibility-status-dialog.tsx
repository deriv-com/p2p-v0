...
const getReasonAction = (reason: string, t: (key: string) => string) => {
  const actionMap: Record<string, { label: string; action: string }> = {
    ...
    advertiser_balance: { label: t("visibilityStatus.actions.addFunds"), action: "add_funds" },
    advertiser_adverts_unlisted: { label: t("visibilityStatus.actions.activateAds"), action: "activate" },
    advertiser_status: { label: t("visibilityStatus.actions.contactSupport"), action: "contact_support" },
