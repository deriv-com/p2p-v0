...
export default function AdDetailsForm({
  ...
  const requestingExchangeRateRef = useRef(false)
  ...
  useEffect(() => {
    ...
    if (requestingExchangeRateRef.current) return

    const requestTimer = setTimeout(() => {
      requestingExchangeRateRef.current = true
    ...
    const unsubscribe = subscribe((data: any) => {
      if (data.options?.channel === `exchange_rates/${buyCurrency}`) {
        if (data.payload?.[forCurrency]?.rate) {
          ...
          requestingExchangeRateRef.current = false
