...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border bg-popover text-start text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
        position === "popper" &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
      className={cn(
      "relative flex w-full cursor-default select-none items-start rounded-sm py-1.5 ps-4 pe-2 text-sm text-start outline-none hover:bg-black hover:text-white hover:[&_.select-item-description]:text-white/80 focus:bg-black focus:text-white focus:[&_.select-item-description]:text-white/80 data-[highlighted]:bg-black data-[highlighted]:text-white data-[highlighted]:[&_.select-item-description]:text-white/80 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className,
    )}
    {...props}
  >
    <SelectPrimitive.ItemText className="w-full text-start">{children}</SelectPrimitive.ItemText>
  </SelectPrimitive.Item>
))
